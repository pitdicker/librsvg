    ...
    default:
        {
            ...
            output.x = rsvg_normalize_length (source->x, ctx, HORIZONTAL) - x;
            output.y = rsvg_normalize_length (source->y, ctx, VERTICAL) - y;
            output.z = rsvg_normalize_length (source->z, ctx, NO_DIR) - z;
            output = normalise (output);
        }
        break;
    }
                  ...
                  gdouble x1, gdouble y1, gdouble z, cairo_matrix_t *affine, RsvgDrawingCtx * ctx)
{
    ...
    sx = rsvg_normalize_length (source->x, ctx, HORIZONTAL);
    sy = rsvg_normalize_length (source->y, ctx, VERTICAL);
    sz = rsvg_normalize_length (source->z, ctx, NO_DIR);
    spx = rsvg_normalize_length (source->pointsAtX, ctx, HORIZONTAL);
    spy = rsvg_normalize_length (source->pointsAtY, ctx, VERTICAL);
    spz = rsvg_normalize_length (source->pointsAtZ, ctx, NO_DIR);
