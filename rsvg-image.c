...
rsvg_node_image_draw (RsvgNode * self, RsvgDrawingCtx * ctx, int dominate)
{
    ...
    x = rsvg_normalize_length (&z->x, ctx, HORIZONTAL);
    y = rsvg_normalize_length (&z->y, ctx, VERTICAL);
    w = rsvg_normalize_length (&z->w, ctx, HORIZONTAL);
    h = rsvg_normalize_length (&z->h, ctx, VERTICAL);
...
rsvg_node_image_set_atts (RsvgNode * self, RsvgHandle * ctx, RsvgPropertyBag * atts)
{
    ...
    if (rsvg_property_bag_size (atts)) {
        ...
        rsvg_set_presentation_props (ctx, image->super.state, "image", klazz, id, atts);
    }
}
...
rsvg_new_image (void)
{
    ...
    image->x = image->y = image->w = image->h = (RsvgLength) {0.0, RSVG_UNIT_NUMBER};
